...
export default async function TripDetailPage({ params }: Props) {
  ...
  let bookings: { id: string; status: string; seats: number; passenger: { id: string; full_name: string; rating: number; phone?: string | null } | null }[] = [];
  if (isDriver) {
    const { data } = await supabase
      .from("bookings")
      .select("id, status, seats, passenger:profiles!passenger_id(id, full_name, rating, phone)")
      .eq("trip_id", id)
